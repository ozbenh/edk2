  ...
  )
{
  ...
  if (!EFI_ERROR (Status)) {
    ...
    Status = ProcessShellParameters (
               &KernelPath, &InitrdPath, &LinuxCommandLine
               );
    if (EFI_ERROR (Status)) {
      goto Error;
    }
    ...
    Status = ProcessAppCommandLine (
               &KernelTextDevicePath, &InitrdTextDevicePath, &LinuxCommandLine
               );
    if (EFI_ERROR (Status)) {
      goto Error;
    }
  ...
  Status = BootLinuxFdt (KernelDevicePath, InitrdDevicePath, AsciiLinuxCommandLine);
