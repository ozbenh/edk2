...
/**
  ...
  @param[out]  Control  Status of the control bits on a serial device :
                        ...
                        . EFI_SERIAL_CLEAR_TO_SEND, EFI_SERIAL_DATA_SET_READY,
  ...
  )
{
  ...
  UINT64 Space;
  UINT64 Events;
  UINT32 Result = EFI_SERIAL_INPUT_BUFFER_EMPTY;

  opal_poll_events(&Events);
  Events = SwapBytes64(Events);
  if (Events & OPAL_EVENT_CONSOLE_INPUT) {
    Result &= ~EFI_SERIAL_INPUT_BUFFER_EMPTY;
  }
  Status = opal_console_write_buffer_space(VTERMNO, &Space);
  if (Status >= 0) {
    Result |= EFI_SERIAL_CARRIER_DETECT;
    Space = SwapBytes64(Space);
    if (Space > 0) {
      Result |= EFI_SERIAL_CLEAR_TO_SEND;
      Result |= EFI_SERIAL_OUTPUT_BUFFER_EMPTY;
    }
  }
  *Control = Result;
  return EFI_SUCCESS;
